        ...
        {insights.map((stat, idx) => {
          ...
          return (
            <div
              key={stat.label}
              role="listitem"
              className={`bg-surface dark:bg-card rounded-[var(--radius-lg)] border border-surface-300 dark:border-muted p-4 flex flex-col items-center text-center opacity-0 animate-[cardEnter_0.4s_ease-out_forwards] ${
                isLast && isOddTotal ? 'col-span-2' : ''
              }`}
              style={{ animationDelay: `${idx * 60}ms` }}
            >
              <div className="w-10 h-10 rounded-full bg-primary-50 dark:bg-surface-primary flex items-center justify-center mb-2">
                <Icon
              ...
              <p className="text-xs font-medium text-primary-700 dark:text-muted-foreground">
                {stat.label}
              </p>
              {stat.detail && (
